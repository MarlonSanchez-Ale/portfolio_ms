...
export default function Project() {
    ...
    const myprojects = [
        ...
        },
        {
            ...
            description: "Test project to handle tasks, manages the registration of tasks, editing, completed tasks, deletion.  Created with Reactjs, Tailwind and global application status management with Redux - Toolkit.",
        ...
        },
        {
            ...
            skill: [{ value: "Wordpress", icon: <BsWordpress size={20} /> }],
        ...
        >
            <div className="flex flex-col justify-center text-center">
                ...
                <div className="grid grid-flow-row gap-8 my-10 p-20 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3">
                    {myprojects && myprojects.map(({ title, image, description, skill, seeMore, url }, index) => (
        ...
        <>
            <Card className="w-full max-w-[24rem] rounded-xl overflow-hidden cursor-pointer shadow-lg hover:shadow-2xl hover:scale-105 transform transition-all duration-500 sm:max-w-[32rem] md:max-w-[24rem]">
                ...
                <CardFooter className="flex items-end justify-between">
                    {skill && skill.map(({ value, icon }, index) => (
                        <Chip value={value} icon={icon} />
                    ))}
                    {seeMore && (
                        <Button
                            size="sm"
